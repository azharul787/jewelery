...
@section('title', 'Customer Report List')
		...
		<div class="col-xs-12 col-sm-12">
			<div class="widget-box">
				<div class="widget-header">
					<h4 class="widget-title"> Sale Report Customer Wise</h4>
				...
				<div class="widget-body">
					<div class="widget-main">
						<form id="demo-form2" action="{{ route('admin.report.csrs')}}" method="post" data-parsley-validate class="form" enctype="multipart/form-data" >
							@csrf
							@method('PUT')
							<div class="row">
										...
										@if($sales != '')
											<div class="row">
												...
												<div class="col-xs-8">
													<div class="text-center">
														...
														<h5>Customer Wise Return Report</h5>
														<p>Customer Name: {{$ty}}</p>
											...
											<table class="table table-condensed table-bordered">
												...
												@foreach($sales as $key=>$pur)
													@php
														$total = $total + $pur->return_price;
														$quantity = $quantity + $pur->quantity;
													@endphp
													<tr>
														<td>{{ $key + 1}}</td>
														<td>{{ date("d-m-Y", strtotime($pur->return_date))}}</td>
														
														<td>{{ $pur->product->product_name}}</td>
														<td>{{ $pur->model_no}}</td>
														<td>{{ $pur->quantity}}</td>
														<td>{{ $pur->return_price}}</td>
													</tr>
												@endforeach
													<tr>
														<th colspan="3"></th>
