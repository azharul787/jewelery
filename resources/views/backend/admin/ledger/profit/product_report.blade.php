		...
		<div class="col-xs-12 col-sm-12">
			<div class="widget-box">
				<div class="widget-header">
					<h4 class="widget-title">Product Wise Profit Report</h4>
				...
				<div class="widget-body">
					<div class="widget-main">
						<form id="demo-form2" action="{{ route('admin.ledger.pprs')}}" method="post" data-parsley-validate class="form" enctype="multipart/form-data" >
							@csrf
							@method('PUT')
							<div class="row">
										...
										@if($sales != '')
											...
											<table class="table table-condensed table-bordered">
												...
												@foreach($sales as $key=>$pur)
													@php
														$total_sale = $total_sale + $pur->unit_price;
														$quantity = $quantity + $pur->quantity;
														$discount = $discount + $pur->discount;
														$profit = $profit + (($pur->unit_price - $pur->purchaseDetail->purchase_price) * $pur->quantity);
														$pp = $pp + $pur->purchaseDetail->purchase_price;
													@endphp
													<tr>
														<td>{{ $key + 1}}</td>
														<td>{{ date("d-m-Y", strtotime($pur->sale_date))}}</td>
														<td>{{ $pur->product->product_name}}</td>
														<td>{{ $pur->purchaseDetail->model_no}}</td>
														<td>{{ $pur->purchaseDetail->purchase_price}}</td>
														<td>{{ $pur->unit_price}}</td>
														<td>{{ $pur->quantity}}</td>
														<td>{{ $pur->discount}}</td>
														<td>{{ ($pur->unit_price - $pur->purchaseDetail->purchase_price) * $pur->quantity}}</td>
													</tr>
												@endforeach
													<tr>
														<th colspan="3"></th>
