										...
										<div class="col-xs-12 col-sm-12">
											...
											<table class="table table-bordered">
												...
												@foreach($data as $key=>$in)
													@php
														$total = $total + $in->income_amount;
														$ex_total = $ex_total + $in->expense_amount;
													@endphp
													<tr>
														<td>{{ $key + 1}}</td>
														<td>{{ date("d-m-Y", strtotime($in->transaction_date))}}</td>
														<td>{{ $in->description}}</td>
														<td>{{ $in->income_amount != NULL ? $in->income_amount : '0.00'}}</td>
														<td>{{ $in->expense_amount != NULL ? $in->expense_amount : '0.00'}}</td>
													</tr>
												@endforeach
													<tr>
														<th colspan="2"></th>
