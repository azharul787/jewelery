...
@section('content')
	<!--<div class="widget-box">
		<div class="widget-header">
			<div class="row">
				...
				<div class="col-sm-2">
					<img src="{{asset('images/logo.png')}}" class="img-responsive" height="50" width="40">
				</div>
				<div class="col-sm-6">
					<h4> Chancle Tractor Spare Parts</h4>
				</div>
				<div class="col-sm-2"></div>
			</div>
		</div>	
	...
	<div class="row">
		...
		<div class="col-xs-12 col-sm-12">
			<div class="widget-box">
				<div class="widget-header">
					<h4 class="widget-title">Loaner List</h4>
				...
				<div class="widget-body">
					<div class="widget-main">
						<div class="row search-section">
							<form action="{{route('admin.loaner.index')}}" id="searchForm" method="Get">
								<div class="col-sm-2">
									<select class="form-control" name="show" id="show">
									{{--<option value="{{$loaners->count()}}">{{$loaners->count()}}</option>--}}
						...
						<div class="table-content">
							<table id="" class="table table-bordered" role="grid" aria-describedby="dynamic-table_info">
								...
								@php
									...
									$credit = 0;
								@endphp
									@foreach($loaners as $key=>$ex)
										<tr>
											<td>{{$key + 1}}</td>
											<td><a href="{{route('admin.loan.show',$ex->id)}}">{{$ex->loaner_name}}</a></td>
											<td>{{$ex->loaner_phone}}</td>
											<td>{{$ex->loaner_address}}</td>
											<td>{{$ex->balance != NUll ? $ex->balance : '0'}}</td>
											<td>
												<a  href="{{route('admin.loaner.edit',$ex->id)}}"><i class="ace-icon fa fa-edit bigger-130"></i></a>
												...
												<form id="delete-form-{{ $ex->id }}" action="{{ route('admin.loaner.destroy',$ex->id) }}" method="POST" style="display: none;">
													@csrf
													@method('DELETE')
												</form> 
											</td>
