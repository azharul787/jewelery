								...
								<tbody>
									@foreach($orders as $key=>$pen)
										<tr>
											...
											<td><a href="{{route('admin.order.customerOrder',$pen->customer_id)}}" title="See Customer Life Time Order Information">{{$pen->customer->customer_name}}</a></td>
											<td><a href="{{route('admin.order.show',$pen->id)}}" title="Details Order Information">{{$pen->order_no}}</a></td>
											<td>{{$pen->total_price}}</td>
											<td>{{$pen->payment}}</td>
											<td>{{$pen->total_price - $pen->payment}}</td>
											<td class="{{ $pen->status == 'Stocked' ? 'green' : 'warning' }}">
											...
											<td>
											@can('order-edit')
												<a  href="{{route('admin.order.edit',$pen->id)}}" title="Edit Order Information"><i class="ace-icon fa fa-edit bigger-130"></i></a>
											@endcan
											@can('order-delete')	
												...
												<form id="delete-form-{{ $pen->id }}" action="{{ route('admin.order.destroy',$pen->id) }}" method="POST" style="display: none;">
													@csrf
													@method('DELETE')
												</form>
											@endcan 
